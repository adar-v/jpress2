...
function initOptionSubmit() {
    ...
    $('#shoppingForm').on('submit', function () {
        $(this).ajaxSubmit({
            type: "post",
            url: jpress.cpath + "/admin/shopping/doSave",
            success: function (data) {
                if (data.state == "ok") {
                    toastr.success('保存成功。');
                } else {
    ...
    $('#newBrand').on('submit',function () {
        $(this).ajaxSubmit({
            type:"post",
            url: jpress.cpath + "/admin/brand/doSave/",
            success:function (data) {
                if (data.state="ok") {
                    toastr.success('保存成功.');
                    // window.location.href = jpress.cpath + "/admin/store/brandManagement"
                }else {
    ...
    $('#newCommodity').on('submit',function () {
        $(this).ajaxSubmit({
            type:"post",
            url: jpress.cpath + "/admin/commodity/doSave/",
            success:function (data) {
                if (data.state="ok") {
                    toastr.success('保存成功.');
                    // window.location.href = jpress.cpath + "/admin/store/brandManagement"
